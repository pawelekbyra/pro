...
export async function verifySession(): Promise<AuthPayload | null> {
  ...
  try {
    ...
    if (
      !authPayload.user?.id ||
      typeof authPayload.user?.sessionVersion !== 'number'
    ) {
    ...
    if (userFromDb.sessionVersion !== authPayload.user.sessionVersion) {
      console.log(
        `Token session version mismatch for user ${userFromDb.id}. DB: ${userFromDb.sessionVersion}, Token: ${authPayload.user.sessionVersion}`
      );
