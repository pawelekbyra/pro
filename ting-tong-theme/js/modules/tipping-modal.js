...
function hideModal() {
    ...
    // Dodaj klasę animacji wyjścia i poczekaj na jej zakończenie
    dom.modal.classList.add('is-hiding');

    // Użyj UI.closeModal, ale upewnij się, że nie koliduje to z naszą animacją
    // przez dodanie niewielkiego opóźnienia i upewnienie się, że klasa 'visible' jest usuwana.
    setTimeout(() => {
        UI.closeModal(dom.modal, {
            // Nie potrzebujemy już specjalnego selektora, bo animacja jest na overlayu
        });
        resetModalState();
    }, 50); // Małe opóźnienie, aby CSS mógł zadziałać
}
