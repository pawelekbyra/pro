...
// =========================================================================
// 6. OBSŁUGA PŁATNOŚCI STRIPE
// =========================================================================
/**
 * AJAX handler to create a Stripe Payment Intent.
 */
function tt_create_payment_intent() {
    // 1. Verify nonce and user login status
    check_ajax_referer('tt_ajax_nonce', 'nonce');
    if (!is_user_logged_in()) {
    ...
    // 4. Validate amount
    if ($amount <= 0) {
        wp_send_json_error(['message' => 'Nieprawidłowa kwota.'], 400);
    }
...
/**
 * AJAX handler for successful tip handling.
 */
function tt_handle_tip_success() {
    // 1. Verify nonce and user login status
    check_ajax_referer('tt_ajax_nonce', 'nonce');
    if (!is_user_logged_in()) {
        wp_send_json_error(['message' => 'Brak autoryzacji.'], 401);
    }
    // 2. Set Stripe API Key
    \Stripe\Stripe::setApiKey(TT_STRIPE_SECRET_KEY);
    // 3. Get data from POST request
    $payment_intent_id = isset($_POST['payment_intent_id']) ? sanitize_text_field($_POST['payment_intent_id']) : '';
    // 4. Validate payment intent ID
    if (empty($payment_intent_id)) {
        wp_send_json_error(['message' => 'Brak ID intencji płatności.'], 400);
    }
    // 5. Verify the Payment Intent on Stripe's end
    try {
        $paymentIntent = \Stripe\PaymentIntent::retrieve($payment_intent_id);
        if ($paymentIntent->status === 'succeeded') {
            // Tutaj możesz dodać logikę po udanej płatności
            // np. zapis do bazy danych, wysłanie e-maila itp.
            wp_send_json_success(['message' => 'Płatność została pomyślnie zweryfikowana.']);
        } else {
            wp_send_json_error(['message' => 'Płatność nie została zakończona pomyślnie.'], 402);
        }
    } catch (Exception $e) {
        wp_send_json_error(['message' => 'Błąd weryfikacji płatności: ' . $e->getMessage()], 500);
    }
}
