...
/**
 * Główna funkcja do tworzenia wszystkich niestandardowych tabel przy aktywacji
motywu.
 */
...
function tt_likes_get_count( $item_id ) {
        global $wpdb;
        return (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpd
b->prefix}tt_likes WHERE item_id = %d", $item_id ) );
}
...
function tt_likes_user_has( $item_id, $user_id ) {
        ...
        global $wpdb;
        return (bool) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wp
db->prefix}tt_likes WHERE item_id = %d AND user_id = %d", $item_id, $user_id ) )
;
}
...
function tt_comments_get_count( $slide_id ) {
        global $wpdb;
        return (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpd
b->prefix}tt_comments WHERE slide_id = %s", $slide_id ) );
}
...
function tt_comment_likes_get_count( $comment_id ) {
        global $wpdb;
        return (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpd
b->prefix}tt_comment_likes WHERE comment_id = %d", $comment_id ) );
}
...
function tt_comment_likes_user_has( $comment_id, $user_id ) {
        ...
        global $wpdb;
        return (bool) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wp
db->prefix}tt_comment_likes WHERE comment_id = %d AND user_id = %d", $comment_id
, $user_id ) );
}
...
function tt_get_simulated_posts() {
        return [
                [
                        ...
                        'video_url'    => 'https://commondatastorage.googleapis.
com/gtv-videos-bucket/sample/BigBuckBunny.mp4',
                        'access'       => 'public',
                        'comments'     => 10,
                        'author'       => [
                                'name'        => 'Big Buck Bunny',
                                'description' => 'Oficjalny profil Big Buck Bunn
y. Zobaczcie moje przygody!',
                                'avatar'      => 'https://i.pravatar.cc/100?u=bu
nny',
                                'is_vip'      => false,
                        ],
                ...
                [
                        ...
                        'video_url'    => 'https://commondatastorage.googleapis.
com/gtv-videos-bucket/sample/ElephantsDream.mp4',
                        'access'       => 'secret',
                        'comments'     => 20,
                        'author'       => [
                                'name'        => 'Elephants Dream',
                                'description' => 'Twórcy filmu "Elephants Dream"
. Dzielimy się kulisami naszej pracy.',
                                'avatar'      => 'https://i.pravatar.cc/100?u=el
ephant',
                                'is_vip'      => false,
                        ],
...
com/gtv-videos-bucket/sample/ForBiggerBlazes.mp4',
                        ...
                        'author'       => [
                                ...
                                'description' => 'Profil poświęcony filmom z efe
ktami specjalnymi. Tylko dla fanów PWA!',
                                'avatar'      => 'https://i.pravatar.cc/100?u=bl
aze',
                                'is_vip'      => false,
                        ],
                        'post_content' => 'Tajemniczy film tylko dla użytkownik
ów PWA.',
                ],
                [
                        'post_id'      => 4,
                        'post_title'   => 'Are You Satisfied (Marina and the Dia
monds cover)',
                        'video_url'    => 'https://commondatastorage.googleapis.
com/gtv-videos-bucket/sample/ForBiggerEscapes.mp4',
                        'access'       => 'public',
                        'comments'     => 15,
                        'author'       => [
                                'name'        => 'Paweł Polutek',
                                'description' => 'Cześć! Jestem Paweł Polutek, e
ntuzjasta technologii webowych i twórca tej aplikacji. Dzielę się tutaj moimi ek
sperymentami i projektami. Zapraszam do oglądania!',
                                'avatar'      => get_template_directory_uri() .
'/assets/img/avatar-pawel-polutek.png',
                                'is_vip'      => true,
                        ],
                        'post_content' => 'Jedna z moich ulubionych piosenek w m
oim wykonaniu. Mam nadzieję, że się Wam spodoba!',
                ],
        ];
}
...
function tt_get_slides_data() {
        ...
        foreach ( $simulated_posts as $post ) {
                $slide_id = 'slide-' . str_pad( $post['post_id'], 3, '0', STR_PA
D_LEFT );
                ...
                $slides_data[] = [
                        ...
                        'isLiked'         => tt_likes_user_has( $post['post_id']
, $user_id ),
...
function tt_enqueue_and_localize_scripts() {
        wp_enqueue_style( 'swiper-css', 'https://cdn.jsdelivr.net/npm/swiper@12.
0.2/swiper-bundle.min.css', [], null );
        wp_enqueue_style( 'tingtong-style', get_stylesheet_uri(), [ 'swiper-css'
 ], null );

        wp_enqueue_script( 'swiper-js', 'https://cdn.jsdelivr.net/npm/swiper@12.
0.2/swiper-bundle.min.js', [], null, true );
        wp_register_script( 'stripe-js', 'https://js.stripe.com/v3/', [], null,
true );
...
function tt_process_ajax_request() {
    ...
    $data = [];
    $content_type = isset($_SERVER["CONTENT_TYPE"]) ? trim($_SERVER["CONTENT_TYP
E"]) : '';
    ...
    if ($is_logged_in) {
        ...
        if (!wp_verify_nonce($nonce, 'tt_ajax_nonce')) {
            return ['success' => false, 'message' => 'Błąd weryfikacji (nonce).
Odśwież stronę.'];
        }
    }
...
') {
            $error_message = 'Użytkownik o podanej nazwie lub e-mailu nie istnie
je.';
        } elseif ($error_code === 'incorrect_password') {
            $error_message = 'Podane hasło jest nieprawidłowe. Sprawdź je i spr
óbuj jeszcze raz.';
        }
        ...
        $user_data = [
            ...
            'email_consent'       => (bool) get_user_meta($u->ID, 'tt_email_cons
ent', true),
            'email_language'      => (string) get_user_meta($u->ID, 'tt_email_la
nguage', true) ?: 'pl',
 ...
 * Ta funkcja jest wywoływana zarówno dla zalogowanych, jak i niezalogowanych u
żytkowników,
 * aby zapewnić spójność stanu po stronie klienta, nawet jeśli sesja wygasła.
 */
function tt_ajax_logout_callback() {
        // Sprawdź nonce, jeśli został dostarczony, ale nie przerywaj, jeśli go
nie ma.
...
add_action( 'wp_ajax_toggle_like', function () {
        check_ajax_referer( 'tt_ajax_nonce', 'nonce' );
        if ( ! is_user_logged_in() ) {
                wp_send_json_error( [ 'message' => 'Musisz się zalogować, aby po
lubić.' ], 401 );
        }
        ...
        if ( tt_likes_user_has( $item_id, $user_id ) ) {
                $wpdb->delete( $table_name, [ 'item_id' => $item_id, 'user_id' =
> $user_id ] );
                $status = 'unliked';
        } else {
                $wpdb->insert( $table_name, [ 'item_id' => $item_id, 'user_id' =
> $user_id ] );
                $status = 'liked';
        }
        wp_send_json_success( [ 'status' => $status, 'count'  => tt_likes_get_co
unt( $item_id ) ] );
} );
...
function tt_generate_mock_comments($count) {
    ...
    for ($i = 1; $i <= $count; $i++) {
        ...
        $comments[] = [
            ...
            'image_url' => null,
            'timestamp' => (new DateTime("-{$i} minutes"))->format(DateTime::ATO
M),
            'likes'     => rand(0, 50),
...
']) : '';
    ...
    $results = $wpdb->get_results($wpdb->prepare(
        "SELECT * FROM {$comments_table} WHERE slide_id = %s ORDER BY created_at
 ASC",
        $slide_id
    ));
    ...
    foreach ($results as $c) {
        ...
        $comments[] = [
            ...
            'user'      => $author_data ? $author_data->display_name : 'Użytkown
ik',
            ...
            'canEdit'   => (int) $c->user_id === $user_id || current_user_can('m
anage_options'),
        ];
    }
...
add_action('wp_ajax_tt_post_comment', function() {
    ...
    $slide_id = isset($_POST['slide_id']) ? sanitize_text_field($_POST['slide_id
']) : '';
    $text = isset($_POST['text']) ? sanitize_textarea_field(wp_unslash($_POST['t
ext'])) : '';
    $parent_id = isset($_POST['parent_id']) && !empty($_POST['parent_id']) ? abs
...
;
    $new_text = isset($_POST['text']) ? sanitize_textarea_field(wp_unslash($_POS
T['text'])) : '';
    ...
    $comments_table = $wpdb->prefix . 'tt_comments';
    $comment = $wpdb->get_row($wpdb->prepare("SELECT user_id FROM {$comments_tab
le} WHERE id = %d", $comment_id));

    if (!$comment || ((int) $comment->user_id !== get_current_user_id() && !curr
ent_user_can('manage_options'))) {
        wp_send_json_error(['message' => 'Brak uprawnień.'], 403);
    }

    $wpdb->update($comments_table, ['content' => $new_text], ['id' => $comment_i
d]);

    // Po aktualizacji, pobierz pełne dane komentarza, aby zwrócić je do fronten
du
    $updated_comment = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$comments_t
able} WHERE id = %d", $comment_id));
    $author_data = get_userdata($updated_comment->user_id);
    $user_id = get_current_user_id();

    $comment_data = [
        'id'        => (int) $updated_comment->id,
        'parentId'  => $updated_comment->parent_id ? (int) $updated_comment->par
ent_id : null,
...
: '',
        ...
        'timestamp' => (new DateTime($updated_comment->created_at))->format(Date
Time::ATOM),
        'likes'     => tt_comment_likes_get_count($updated_comment->id),
        'isLiked'   => tt_comment_likes_user_has($updated_comment->id, $user_id)
,
        'canEdit'   => (int) $updated_comment->user_id === $user_id || current_u
ser_can('manage_options'),
    ];
...
;
    ...
    $comments_table = $wpdb->prefix . 'tt_comments';
    $comment = $wpdb->get_row($wpdb->prepare("SELECT user_id, slide_id FROM {$co
mments_table} WHERE id = %d", $comment_id));
...
ent_user_can('manage_options'))) {
    ...
    $wpdb->delete($wpdb->prefix . 'tt_comment_likes', ['comment_id' => $comment_
id]);
...
;
    ...
    if (tt_comment_likes_user_has($comment_id, $user_id)) {
        $wpdb->delete($likes_table, ['comment_id' => $comment_id, 'user_id' => $
user_id]);
        $isLiked = false;
    } else {
        $wpdb->insert($likes_table, ['comment_id' => $comment_id, 'user_id' => $
user_id]);
        $isLiked = true;
    }
...
add_action('wp_ajax_tt_upload_comment_image', function() {
    ...
    if (!isset($_FILES['image']) || $_FILES['image']['error'] !== UPLOAD_ERR_OK)
 {
    ...
    if (!in_array($file['type'], $allowed_types)) {
        wp_send_json_error(['message' => 'Nieprawidłowy typ pliku. Dozwolone: JP
G, PNG, GIF, WEBP'], 400);
    }

    // Walidacja rozmiaru (max 5MB)
    if ($file['size'] > 5 * 1024 * 1024) {
        wp_send_json_error(['message' => 'Plik jest za duży. Maksymalny rozmiar:
 5MB'], 400);
    }
    ...
    $attachment = [
        ...
        'post_title' => sanitize_file_name(pathinfo($movefile['file'], PATHINFO_
FILENAME)),
        'post_content' => '',
        'post_status' => 'inherit'
    ];
    ...
    if (is_wp_error($attach_id)) {
        wp_send_json_error(['message' => 'Nie udało się utworzyć załącznika.'],
500);
    }
...
add_action('wp_ajax_tt_create_stripe_payment_intent', 'tt_create_stripe_payment_
intent_callback');
add_action('wp_ajax_nopriv_tt_create_stripe_payment_intent', 'tt_create_stripe_p
ayment_intent_callback');
function tt_create_stripe_payment_intent_callback() {
    ...
    $currency = isset($data['currency']) ? strtolower(sanitize_text_field($data[
'currency'])) : 'pln';

    // FIX: Sprawdzenie minimalnej kwoty (1 jednostka waluty)
    if ($amount_pln < 1) {
        $currency_display = strtoupper($currency);
        wp_send_json_error(['message' => "Kwota napiwku musi wynosić co najmniej
 1 {$currency_display}."], 400);
        return;
    }
    ...
    // Kwota musi być przekazana w najmniejszej jednostce waluty (centach/grosza
ch)
    $amount_cents = round($amount_pln * 100);

    if (!class_exists('\Stripe\Stripe')) {
        wp_send_json_error(['message' => 'Błąd integracji płatności. Skontaktuj
się z administratorem.']);
        return;
    }
    if (empty(TT_STRIPE_SECRET_KEY)) {
        wp_send_json_error(['message' => 'Brak konfiguracji płatności po stronie
 serwera. Sprawdź TT_STRIPE_SECRET_KEY.'], 500);
        return;
    }
    ...
    try {
        ...
        wp_send_json_error(['message' => 'Błąd Stripe API: ' . $e->getMessage()
. ' Kod: ' . $e->getStripeCode()], 500);
    }
}
...
add_action('wp_ajax_tt_profile_update', function () {
    ...
    $u = wp_get_current_user();
    $first = isset($_POST['first_name']) ? sanitize_text_field( wp_unslash($_POS
T['first_name']) ) : '';
    $last  = isset($_POST['last_name'])  ? sanitize_text_field( wp_unslash($_POS
T['last_name']) )  : '';
    $email = isset($_POST['email'])      ? sanitize_email( wp_unslash($_POST['em
ail']) ) : '';
    ...
    if (is_wp_error($res)) {
        wp_send_json_error(['message' => $res->get_error_message() ?: 'Błąd aktu
alizacji.'], 500);
    }
    wp_send_json_success([
        'display_name' => $display_name ?: $u->display_name,
...
add_action('wp_ajax_tt_avatar_upload', function () {
    try {
        check_ajax_referer('tt_ajax_nonce', 'nonce');
        if ( ! is_user_logged_in() ) {
            wp_send_json_error(['message' => 'Brak uprawnień: Musisz być zalogow
any.'], 401);
        }
...
: '';
        if (empty($dataUrl) || strpos($dataUrl, 'data:image') !== 0) {
            wp_send_json_error(['message' => 'Błąd danych: Nieprawidłowy format
data URL.'], 400);
        }

        if ( ! preg_match('#^data:(image/(?:png|jpeg|gif));base64,(.+)$#', $data
Url, $matches) ) {
            wp_send_json_error(['message' => 'Błąd formatu: Oczekiwano obrazu PN
G, JPEG lub GIF.'], 400);
        }

        $mime = strtolower($matches[1]);
        $bin = base64_decode($matches[2]);
        if ( ! $bin || strlen($bin) > 5 * 1024 * 1024) { // Zwiększony limit do
5MB
            wp_send_json_error(['message' => 'Błąd pliku: Obraz jest uszkodzony
lub za duży (limit 5MB).'], 400);
        }
        ...
        $u = wp_get_current_user();
        $ext = ($mime === 'image/png') ? 'png' : 'jpg'; // Uproszczenie dla najc
zęstszych typów
        ...
        if ($write_result === false) {
            wp_send_json_error(['message' => 'Błąd zapisu: Nie można zapisać pli
ku tymczasowego.'], 500);
        }

        $file_array = ['name' => $filename, 'type' => $mime, 'tmp_name' => $tmp_
path, 'error' => 0, 'size' => filesize($tmp_path)];
        $file = wp_handle_sideload($file_array, ['test_form' => false]);

        if (isset($file['error'])) {
            @unlink($tmp_path);
            wp_send_json_error(['message' => 'Błąd WordPress: ' . $file['error']
], 500);
        }

        $attach_id = wp_insert_attachment(['post_mime_type' => $mime, 'post_titl
e' => $filename, 'post_content' => '', 'post_status' => 'inherit'], $file['file'
]);
        if (is_wp_error($attach_id)) {
            @unlink($file['file']);
            wp_send_json_error(['message' => 'Błąd bazy danych: Nie można utworz
yć załącznika.'], 500);
        }
...
tMessage(), 'trace' => $e->getTraceAsString()], 500);
        } else {
            wp_send_json_error(['message' => 'Wystąpił nieoczekiwany błąd serwer
a.'], 500);
        }
    }
});
add_action('wp_ajax_tt_password_change', function () {
    ...
    $u = wp_get_current_user();
    $cur = isset($_POST['current_password']) ? (string) wp_unslash($_POST['curre
nt_password']) : '';
    $n1  = isset($_POST['new_password_1'])   ? (string) wp_unslash($_POST['new_p
assword_1'])   : '';
    $n2  = isset($_POST['new_password_2'])   ? (string) wp_unslash($_POST['new_p
assword_2'])   : '';
    if (empty($cur) || empty($n1) || empty($n2) || $n1 !== $n2 || strlen($n1) <
8) {
        wp_send_json_error(['message' => 'Sprawdź pola hasła (min. 8 znaków, mus
zą być identyczne).'], 400);
    }
    require_once ABSPATH . 'wp-includes/pluggable.php';
    if (!wp_check_password($cur, $u->user_pass, $u->ID)) {
        wp_send_json_error(['message' => 'Aktualne hasło jest nieprawidłowe.'],
403);
    }
...
add_action('wp_ajax_tt_account_delete', function () {
    ...
    $u = wp_get_current_user();
    $confirm = isset($_POST['confirm_text']) ? trim((string) wp_unslash($_POST['
confirm_text'])) : '';
    if ($confirm !== 'USUWAM KONTO') {
        wp_send_json_error(['message' => 'Aby potwierdzić, wpisz dokładnie: USUW
AM KONTO'], 400);
    }
    if (user_can($u, 'administrator')) {
        wp_send_json_error(['message' => 'Konto administratora nie może być usun
ięte.'], 403);
    }
...
add_shortcode( 'tt_login_form', function() {
        if ( is_user_logged_in() ) {
                return '<p style="padding: 20px; text-align: center;">Jesteś ju
ż zalogowany.</p>';
        }
        return '<form name="loginform" class="login-form" action="#" method="pos
t">
        <p><label for="user_login">Nazwa użytkownika lub e-mail</label><input ty
pe="text" name="log" id="user_login" class="input" value="" size="20" required a
utocomplete="username"></p>
        <p><label for="user_pass">Hasło</label><input type="password" name="pwd"
 id="user_pass" class="input" value="" size="20" required autocomplete="current-
password"></p>
        <p><input type="submit" name="wp-submit" id="wp-submit" class="button bu
tton-primary" value="Zaloguj się"></p>
    </form>';
});
...
add_filter('get_avatar_url', function ($url, $id_or_email, $args) {
    ...
    elseif (is_object($id_or_email) && isset($id_or_email->user_id)) $user_id =
(int) $id_or_email->user_id;
    elseif (is_string($id_or_email) && ($user = get_user_by('email', $id_or_emai
l))) $user_id = (int) $user->ID;

    // Definicja Twojego domyślnego pliku
    $default_avatar_url = get_template_directory_uri() . '/assets/img/default-us
er.png';
    ...
    // KROK 3: Dla wszystkich innych scenariuszy, gdzie WordPress normalnie uży
łby Gravatara/domyślnego WP
    if (strpos($url, 'gravatar.com') !== false || strpos($url, 's.w.org') !== fa
lse) {
        return $default_avatar_url;
    }
...
add_action('wp_ajax_tt_complete_profile', function () {
    // 1. Bezpieczeństwo i walidacja wstępna - teraz używa standardowego nonce z
 POST
    check_ajax_referer('tt_ajax_nonce', 'nonce');
    if (!is_user_logged_in()) {
        wp_send_json_error(['message' => 'Brak autoryzacji. Musisz być zalogowan
y.'], 401);
    }
    ...
    $first_name = isset($data['first_name']) ? sanitize_text_field(wp_unslash($d
ata['first_name'])) : '';
    $last_name = isset($data['last_name']) ? sanitize_text_field(wp_unslash($dat
a['last_name'])) : '';
    $new_password = isset($data['new_password']) ? wp_unslash($data['new_passwor
d']) : '';
    // Zmiana: 'true'/'false' jako string z FormData
    $email_consent = isset($data['email_consent']) ? filter_var($data['email_con
sent'], FILTER_VALIDATE_BOOLEAN) : false;
    $email_language = isset($data['email_language']) && in_array($data['email_la
nguage'], ['pl', 'en']) ? $data['email_language'] : 'pl';

    if (empty($first_name) || empty($last_name)) {
        wp_send_json_error(['message' => 'Imię i nazwisko są polami wymaganymi.'
], 400);
    }
    ...
    if (!empty($new_password)) {
        if (strlen($new_password) < 8) {
            wp_send_json_error(['message' => 'Hasło musi zawierać co najmniej 8
znaków.'], 400);
        }
    ...
    if (is_wp_error($result)) {
        wp_send_json_error(['message' => $result->get_error_message() ?: 'Nie ud
ało się zaktualizować profilu.'], 500);
    }
    ...
    $updated_user_data = [
        ...
        'username'            => $u->user_login,
        'email'               => $u->user_email,
        'display_name'        => $user_data_to_update['display_name'] ?: $u->dis
play_name,
        'first_name'          => $first_name,
        'last_name'           => $last_name,
        'avatar'              => get_avatar_url($u->ID, ['size' => 96]),
