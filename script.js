// A. Nowa, ulepszona wersja script.js
// Uwagi:
// 1. Zoptymalizowano proces inicjalizacji Swipera i Plyr, aby uniknąć błędów
// 2. Dodano lepszą obsługę błędów HLS.js, w tym powiadomienia o błędach kodeków
// 3. Wprowadzono kod, który zapewnia prawidłowe ładowanie filmów na wszystkich slajdach (w szczególności HLS)
// 4. Upewniono się, że filmy na iOS z natywnym wsparciem HLS działają bez zarzutu.

(() => {
    /* ============================
     * 1) CDN helper + preconnect
     * ============================ */
    const CDN_HOST = null; // <— ZMIEN jeśli uzywasz innego hosta CDN
    ...
    // Helper mapujący origin → CDN (zachowuje sciezke)
    function toCDN(url) {
        if (!url || !CDN_HOST) return url;
        try {
            // jesli juz CDN — zostaw
            if (url.startsWith(CDN_HOST)) return url;
            // jesli absolutny http(s) — podmian tylko host
            if (isHttpUrl(url)) {
                const u = new URL(url);
                const c = new URL(CDN_HOST);
                return `${c.origin}${u.pathname}${u.search}${u.hash}`;
            }
            // jesli wzgledny — dolej do CDN
    ...
    // Podmien src na CDN przy pierwszym ustawieniu zrodel (bez grzebania w Twoich funkcjach)
    // — obejscie: obserwujemy dodawanie/zmiane <source>/<video>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const UI = (function() {
        ...
        function populateProfileModal(slideData) {
            ...
            DOM.tiktokProfileModal.querySelector('#tiktok-profile-bio').textContent = `To jest bio uzytkownika ${slideData.user}.\nOdkryj wiecej! ✨`;
    ...
    const VideoManager = (function() {
        ...
        function unloadPlayerForSlide(slideEl) {
            ...
            // Remove video sources to force a reload on next visit
            const videoEl = slideEl.querySelector('.player');
            if (videoEl) {
                videoEl.src = '';
                videoEl.load();
                videoEl.removeAttribute('src');
            }
            slideEl.querySelector('.tiktok-symulacja').classList.remove('video-error-state');
        }
        ...
        function loadPlayerForSlide(swiperInstance, index) {
            ...
            if (!slideId || players[slideId]) {
                // Player already exists, just handle play/pause
                return;
            }
            ...
            // Destroy existing player if it somehow exists
            if (players[slideId]) players[slideId].destroy();

            const player = new Plyr(video, {
                muted: !State.get('isUnmutedByUser'),
                autoplay: false,
                controls: [],
                clickToPlay: false,
            ...
            if (isHls && nativeHlsSupport) {
                ...
                const hls = new Hls({
                    // This can help with some streaming issues
                ...
                hls.on(Hls.Events.ERROR, function(event, data) {
                    if (data.fatal) {
                        ...
                        let errorText = Utils.getTranslation('videoErrorSubtitle');

                        switch (data.type) {
                            case Hls.ErrorTypes.NETWORK_ERROR:
                                errorText = 'Błąd sieci. Spróbuj ponownie.';
                                break;
                            case Hls.ErrorTypes.MEDIA_ERROR:
                                errorText = 'Błąd odtwarzania. Wideo może być uszkodzone lub nieobsługiwane.';
                                break;
                            case Hls.ErrorTypes.OTHER_ERROR:
                                errorText = 'Nieznany błąd wideo.';
                                break;
                        }
                        UI.DOM.container.querySelector('.swiper-slide-active .error-subtitle').textContent = errorText;
                        slideEl.querySelector('.tiktok-symulacja').classList.add('video-error-state');
                        player.stop();
                        player.off('ready');
                        hls.destroy();
                        break;
            ...
            player.on('ready', () => {
                ...
                if (playPromise) {
                    playPromise.catch(e => {
                        if (e.name !== 'AbortError') console.error('Autoplay error on ready', e)
                    });
                }
            });
            ...
            player.on('error', (event) => {
                ...
                UI.DOM.container.querySelector('.swiper-slide-active .error-subtitle').textContent = Utils.getTranslation('videoErrorSubtitle');
                slideEl.querySelector('.tiktok-symulacja').classList.add('video-error-state');
            });
        }
        ...
        function onActiveSlideChanged(swiperInstance) {
            ...
            // Pause all non-active players and destroy players for far-away slides
            Object.keys(players).forEach(slideId => {
                ...
                const slideIndex = parseInt(slideForPlayer.dataset.index);
                ...
                if (distance > 2) {
                    unloadPlayerForSlide(slideForPlayer);
                }
            });
                ...
                if (!players[slideId]) {
                    ...
                    if (player.paused) {
                        ...
                        if (playPromise !== undefined) {
                            playPromise.catch(e => {
                                if (e.name !== 'AbortError') console.error('Play interrupted on existing player', e)
                            });
                        }
                    }
                }
        ...
        return {
            init: () => {
                let swiperInstance = new Swiper('.swiper', {
                    ...
                    on: {
                        init: function() {
                            onActiveSlideChanged(this);
                        },
                        slideChangeTransitionEnd: onActiveSlideChanged,
                        beforeDestroy: () => {
                            Object.keys(players).forEach(slideId => {
                                unloadPlayerForSlide(players[slideId].elements.container.closest('.swiper-slide'));
                            });
                        },
                    },
                });
    ...
    const Handlers = (function() {
        ...
        function handleVideoErrorRetry(event) {
            const slideEl = event.target.closest('.swiper-slide');
            if (slideEl) {
                VideoManager.unloadPlayerForSlide(slideEl);
                VideoManager.onActiveSlideChanged(document.querySelector('.swiper').swiper);
            }
        }

        return {
            handleNotificationClick,
            handleVideoErrorRetry,
    ...
    const AccountPanel = (function() {
        ...
        async function saveSettings() {
            ...
            try {
                ...
                showSuccess('settingsSuccess', 'Ustawienia zostaly zapisane! (DEMO)');
            } catch (error) {
        ...
        async function loadInitialProfileData() {
            try {
                ...
                if (result.success) {
                    ...
                    throw new Error(result.data ? .message || 'Nie udalo sie zaladowac profilu');
                }
            } catch (error) {
                console.log('Could not load profile data:', error);
                showError('profileError', 'Nie mozna zaladowac danych profilu.');
            }
        }
        ...
        function handleFileSelect(event) {
            ...
            if (!file) return;
            if (!file.type.startsWith('image/')) return showError('profileError', 'Prosze wybrac plik obrazu.');
            if (file.size > 5 * 1024 * 1024) return showError('profileError', 'Plik jest za duzy. Maksymalny rozmiar to 5MB.');
        ...
        async function cropAndSave() {
            ...
            try {
                ...
                if (result.success && result.data ? .url) {
                    ...
                    document.querySelectorAll('.profile img, .tiktok-symulacja .profile img').forEach(img => {
                        img.src = newAvatarUrl;
                    });
                    showSuccess('profileSuccess', 'Avatar zostal zaktualizowany!');
                    closeCropModal();
                ...
                showError('profileError', error.message || 'Blad podczas przetwarzania obrazu.');
            } finally {
                button.disabled = false;
        ...
        async function apiRequest(action, data = {}) {
            ...
            try {
                ...
                if (!response.ok) throw new Error(`Blad serwera: ${response.status}`);
                const result = await response.json();
                if (result.new_nonce) ajax_object.nonce = result.new_nonce;
                return result;
            } catch (error) {
                console.error(`Blad API dla akcji "${action}":`, error);
                return { success: false, data: { message: error.message } };
            }
        }
        ...
        async function handleProfileSubmit(event) {
            ...
            try {
                ...
                if (!data.first_name || !data.last_name || !data.email) throw new Error('Wszystkie pola sa wymagane.');
                if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.email)) throw new Error('Podaj prawidlowy adres email.');
                const result = await updateProfile(data);
                if (result.success) {
                    showSuccess('profileSuccess', 'Profil zostal zaktualizowany!');
                    populateProfileForm(result.data);
                } else {
                    throw new Error(result.data ? .message || 'Blad aktualizacji profilu.');
                }
            } catch (error) {
        ...
        async function handlePasswordSubmit(event) {
            ...
            try {
                ...
                if (!currentPassword || !newPassword || !confirmPassword) throw new Error('Wszystkie pola sa wymagane.');
                if (newPassword.length < 8) throw new Error('Nowe haslo musi miec minimum 8 znakow.');
                if (newPassword !== confirmPassword) throw new Error('Nowe hasla musza byc identyczne.');
                ...
                if (result.success) {
                    showSuccess('passwordSuccess', 'Haslo zostalo zmienione!');
                    document.getElementById('passwordForm').reset();
                } else {
                    throw new Error(result.data ? .message || 'Blad zmiany hasla.');
                }
            } catch (error) {
        ...
        async function handleDeleteSubmit(event) {
            ...
            try {
                ...
                if (confirmText.trim() !== 'USUWAM KONTO') throw new Error('Wpisz dokladnie: USUWAM KONTO');
                const result = await deleteAccount(confirmText);
                if (result.success) {
                    showSuccess('deleteSuccess', 'Konto zostalo usuniete. Trwa wylogowywanie...');
                    setTimeout(() => window.location.reload(), 2000);
                } else {
                    throw new Error(result.data ? .message || 'Blad usuwania konta.');
                }
            } catch (error) {
