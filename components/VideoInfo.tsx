...
const VideoInfo: React.FC<VideoInfoProps> = ({ user, description }) => {
  ...
  const widthClass = isExpanded
    ...
    : isClamped
      ...
      : 'right-3'; // Collapsed description adjusts dynamically

  return (
    <>
      {/* Transparent overlay to close the description when clicking outside of it */}
      {isExpanded && (
        <div className="fixed inset-0 z-[106]" onClick={handleCollapse} />
      )}
      ...
      >
        <div
          className={`transition-all duration-300 ease-in-out ${isExpanded ? 'bg-black/40 backdrop-blur-sm rounded-lg p-4 max-h-[40vh] overflow-y-auto cursor-default' : 'max-h-[80px] overflow-hidden'}`}
        >
